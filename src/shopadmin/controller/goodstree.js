...
export default class extends Base {
  ...
  async pageAction() {
    ...
    let node = await this.model("shopadmin/tree").page(pid);
  ...
  async pageallAction() {
    ...
    let node = await this.model("shopadmin/tree").pageall();
    return this.json(node);
  }
  //编辑节点
  editAction() {
  ...
  async getAction() {
    ...
    let node = await this.model("shopadmin/tree").get(this.post("id"));
    return this.json(node);
  }
  ...
  async updateAction() {
    ...
    let node = await this.model("shopadmin/tree").nodeUpdate(p);
    if (node.state == true) {
      return this.json({
        "state": true
      });
  ...
  async createAction() {
    ...
    let node = await this.model("shopadmin/tree").create(p);
    if (node.state == true) {
      return this.json({
        "state": true
      });
