...
export default class extends think.model.base {
    init(...args) {
            super.init(...args);
            this.tableName = "goods_node"; //将对应的数据表名设置为 user2
        }
        /**
         *  根据父节点获取所有子节点
     ...
     */
    async get(nodeid) {
      let row = await this.where({id: nodeid}).find();
      return {
        state: true,
        msg:row
      }
     ...
     */
    async create(node) {
      let row = await this.where({id: node.pid}).field("nodepath").find();
      if(!think.isEmpty(row)) {
        let id = await this.add(node);
        let nodepath = row.nodepath + id + ',';
        let eff = await this.where({id: id}).update({nodepath: nodepath});
        return {
          state: true,
          msg: id
        }
